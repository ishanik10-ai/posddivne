...
function buildPrint(){
  ...
  $("printArea").innerHTML=`<div class="receipt"><h2>DESIRED DIVINE</h2><div class="center">CASH MEMO</div><hr><div>Name: ${esc($("customerName").value)}</div><div>Phone: ${esc($("customerPhone").value)}</div><div>L.Digit: ${esc($("customerDigit").value)}</div><div>Memo: #${num}</div><hr>${rows}<hr><div class="summary"><div>Product Total:</div><div>${printMoney(subtotal)}</div><div>Shipping:</div><div>${printMoney(shipping)}</div><div>Discount:</div><div>- ${printMoney(discount)}</div><div class="big">TOTAL:</div><div class="big">${printMoney(total)}</div><div>Advance:</div><div>${printMoney(advance)}</div><div class="big">DUE:</div><div class="big">${printMoney(due)}</div></div><hr><div class="center">Thank You!</div></div>`;
